...
const { existsAsync, readFileAsync, writeFileAsync, statAsync, unlinkAsync, mkdirpAsync } = require('asfs')
...
function create (dirname, options = {}) {
  ...
  return [
    ...
    }),
    {
      ...
      /** Get file content */
      'HEAD': co.wrap(function * middleware (ctx) {
        debug('handle')
        let { filename } = ctx.state
        let exists = yield existsAsync(filename)
        ctx.body = null
        ctx.status = exists ? 204 : 404
      }),
      /** Create a file */
